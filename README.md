...
# Lovdata RAG-agent MCP-server

En Model Context Protocol (MCP) server som fungerer som en bro mellom språkmodeller (som Claude) og norske lov- og forskriftstekster fra Lovdata. Serveren gir språkmodeller mulighet til å søke semantisk, finne relevante lover og hente ut fulle lovtekster via et strukturert grensesnitt.

## Funksjonalitet

MCP-serveren eksponerer følgende verktøy:

1. **semantic_search**: Søk etter relevante lovtekster basert på spørsmål eller søkeord
   - Input: 
...
2. **get_document**: Hent komplett lovtekst basert på ID
   - Input: 
     - `id` (string): ID for dokumentet som skal hentes
   - Output: Fullstendig lovtekst som ren tekst med metadata

## Arkitektur

Systemet bygger på en LangGraph-basert RAG-løsning som håndterer:
- Semantisk søk i vektoriserte lovtekster
- Håndtering av komplekse forespørsler via en forskningsagent
- Strukturert respons med relevante lovparagrafer

MCP-serveren fungerer som et grensesnitt mellom Claude og RAG-systemet, og håndterer kommunikasjon via stdio.
...
- Python 3.9+
- Pinecone-konto (eller annen vektorbase som støttes: Elasticsearch, MongoDB)
- OpenAI API-nøkkel for embeddings
- Tilgang til lovdata-tekster
...
2. Installer avhengigheter:
   ...
   ```
3. Opprett en `.env`-fil med følgende miljøvariabler:
   ```
   # API-nøkler
   PINECONE_API_KEY=din_pinecone_api_nøkkel_her
   OPENAI_API_KEY=din_openai_api_nøkkel_her
   
   # Pinecone konfigurasjon
   PINECONE_INDEX_NAME=lovdata-index
...
Kjør test-skriptet for å teste MCP-serveren lokalt:
...
2. **Retrieval Graph** (`src/retrieval_graph/`)
   - Håndterer semantisk søk i vektoriserte lovtekster
   - Genererer forskningsplaner og henter relevante dokumenter

3. **Konfigurasjon** (`src/config.py`)
   - Håndterer miljøvariabler og konfigurasjon

### Dataflyt

1. Claude sender en forespørsel til MCP-serveren via stdio
2. MCP-serveren forbereder forespørselen og sender den til RAG-systemet
3. RAG-systemet:
   - Analyserer forespørselen
   - Utfører semantisk søk i vektorbasen
   - Henter relevante dokumenter
4. MCP-serveren returnerer resultatene til Claude

## Oppsett av Vektorbase

For å sette opp Pinecone-indeksen:

1. Opprett en konto på Pinecone
2. Lag en ny indeks med konfigurasjon:
   - Dimensjon: 1536 (for OpenAI embeddings)
   - Metrisk: Cosine similarity
3. Sett PINECONE_API_KEY og PINECONE_INDEX_NAME i .env-filen

## Indeksering av Lovdata

For å indeksere lovtekster i vektorbasen, må du:

1. Hente lovtekster fra Lovdata
2. Parse tekstene for å ekstrahere relevante segmenter og metadata
3. Generer embeddings og lagre i vektorbasen

(Merk: Detaljert indekseringsprosess er under implementasjon)
